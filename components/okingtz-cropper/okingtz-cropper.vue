<template>
	<view class="container">
		<view class="page-body uni-content-info">
			...
			<canvas canvas-id="myCanvas" v-if="originalW>0 && originalH>0"
				:style="'position:absolute;border: 1px solid red; width:'+(original?originalW:imageW)+'px;height:'+(original?originalH:imageH)+'px;top:-9999px;left:-9999px;'"></canvas>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			getImageInfo() {
				...
				ctx.drawImage(_this.imageSrc, 0, 0, _this.original ? ORIGINAL_W : IMG_REAL_W, _this.original ? ORIGINAL_H :
					IMG_REAL_H);
				ctx.draw(true, () => {
					// 获取画布要裁剪的位置和宽度   均为百分比 * 画布中图片的宽度    
					// 保证了在微信小程序中裁剪的图片模糊  位置不对的问题 canvasT = (_this.cutT / _this.cropperH) * (_this.imageH / pixelRatio)
					var canvasW = ((_this.cropperW - _this.cutL - _this.cutR) / _this.cropperW) * (_this.original ?
						ORIGINAL_W : IMG_REAL_W);
					var canvasH = ((_this.cropperH - _this.cutT - _this.cutB) / _this.cropperH) * (_this.original ?
						ORIGINAL_H : IMG_REAL_H);
			...
			dragMoveTopAndRight(e) {
				...
				if (_this.fixed) {
					...
					if ((CUT_T - dragLengthY) > (_this.cropperH - _this.cutB)) dragLengthY = CUT_T - (_this.cropperH -
						_this.cutB)
			...
			dragMoveLeftAndBottom(e) {
				...
				if (_this.fixed) {
					...
					if ((CUT_L - dragLengthX) > (_this.cropperW - _this.cutR)) dragLengthX = CUT_L - (_this.cropperW -
						_this.cutR)
